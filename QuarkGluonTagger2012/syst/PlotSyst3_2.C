...
int PlotSyst3_2(float ptmin=30,float ptmax=50,float rhomin=0, float rhomax=15,float etamin=0, float etamax=2.0,const char*var="QGLHisto")
{
...
//string varName="QGLHisto";
 ...
 t_data->Add("/Users/andreamarini/Documents/QGDiscriminator/ZJet2/ZJet_Double*.root");
 t_mc->Add("/Users/andreamarini/Documents/QGDiscriminator/ZJet2/ZJet_DYJets*.root");
...
float PUWeight;t_mc->SetBranchAddress("PUWeight",&PUWeight);
int   pdgIdJet0;t_mc->SetBranchAddress("pdgIdPartJet0",&pdgIdJet0);

float llPt;t_mc->SetBranchAddress("llPt",&llPt);
...
//		cout<<"8 ";cut_flow->Fill(8);
	...
	//Z SELECTION ALREADY DONE IN NTUPLES FROM Z+JET ANALYSIS
	//high stat sample
	if (!(llPt>50))continue;
	//normal selection
...
//		cout<<"--- "<<endl;cut_flow->Fill(9);
	h_mc1->Fill( Syst(varName.c_str(), ptJet0,  rhoPF,  etaJet0,pdgIdJet0,(varName=="QGLHisto")?QGLHisto:QGLMLP),eventWeight*PUReWeight);
...
//	h_mc0_redone->SetLineColor(kGreen);
	...
	TH1F* h_mc0_ERRORS=(TH1F*)h_mc0->Clone("ERRORS");	
...
//	PaintOverflow(h_mc0_redone,"HIST SAME");
	...
	c0->SaveAs(Form("Results/%s_%s_pt%.0f_%.0f_rho%.0f_%.0f_eta%.0f.pdf",Version.c_str(),varName.c_str(),PtMin,PtMax,RhoMin,RhoMax,EtaMax));
